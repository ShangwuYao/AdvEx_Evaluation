...
def mnist_tutorial_jsma(train_start=0, train_end=60000, test_start=0,
	...
	if os.path.exists(model_path + '.meta'): # load
	...
	perturbation_L2s = np.zeros((nb_classes, source_samples), dtype='f')
	...
	for sample_ind in xrange(0, source_samples):
		...
		for target in target_classes:
			...
			adv_x = attackmethod.generate_np(sample, **attack_params) # (1, 28, 28, 1)
			...
			perturbation_L2s[target, sample_ind] = np.mean(np.sum((adv_x - sample)**2,
                                       axis=(1, 2, 3))**.5) # TODO: do this in array
	...
	percent_perturbed_L2 = np.mean(perturbation_L2s)
